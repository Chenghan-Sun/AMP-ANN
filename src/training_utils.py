...
class TrainTools:
    ...
    def force_decompose(self, rel_option=False, norm_option=False, normalize=False):
        """
        Summary:
        ...
        if not normalize:  # default normalize=False
    ...
    def fitting_energy_plot(e_dft_train, e_dft_valid, e_amp_train, e_amp_valid, fig_title):
        ...
        plt.title('RMSE train=%s, valid=%s' % (np.round(rms_train, decimals=2), np.round(rms_valid, decimals=2)))
        ...
        return np.round(rms_train, decimals=2), np.round(rms_valid, decimals=2)
